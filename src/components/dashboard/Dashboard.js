...
import { deleteTask, deleteAllOutDatedTasks } from '../../store/actions/taskActions';
import { deleteAllOutDatedNotifs, deleteNotif } from '../../store/actions/notifActions';
...
export class Dashboard extends Component {
    ...
    render() {
        ...
        let tasksList = tasks && tasks.map(task => {
            if (sortBy !== 'all_groups') {
                if (task.group === sortBy || task.group === 'all') {
                    return <Task handleTaskAdmin={this.handleTaskAdmin} task={task} key={task.id} groupName={this.getNameOfGroup(task)} id_group={task.group} text_date={task.text_date} date={task.date} />
                }
            } else {
                return <Task handleTaskAdmin={this.handleTaskAdmin} task={task} key={task.id} groupName={this.getNameOfGroup(task)} id_group={task.group} text_date={task.text_date} date={task.date} />
            }
        ...
        let notificationsList = notifications && notifications.map((notif, index) => {
            
            return <Notification handleNotifAdmin={this.handleNotifAdmin} key={notif.id} notif={notif} id={index + 1} date={notif.date} text_date={notif.text_date} />
        });
        if (tasks && tasksList) tasksList.map(task => task !== null);
        if (notificationsList && !notificationsList.length) notificationsList = 'Brak powiadomień :)'
            ...
            <div>
                                ...
                                {tasksList.length ? tasksList : (
                                    <div className="card none-tasks">
                                        <div className="card-content black-text">
                                            ...
                                            <p>Mimo wszystko weź pod uwagę, że zadanie mogło nie zostać dodane!</p>
                                        </div>
                ...
                <div id="modal1" className="modal">
                    ...
                    <div className="modal-footer">
                        <a href="#!" className="green white-text modal-close waves-effect waves-green btn-flat">Anuluj</a>
                        <a href="#!" onClick={this.deleteTask} className="red white-text modal-close waves-effect waves-green btn-flat">Usuń</a>
                    </div>
                </div>
                <div id="modal2" className="modal">
                    ...
                    <div className="modal-footer">
                        <a href="#!" className="green white-text modal-close waves-effect waves-green btn-flat">Anuluj</a>
                        <a href="#!" onClick={this.deleteNotif} className="red white-text modal-close waves-effect waves-green btn-flat">Usuń</a>
                    </div>
                </div>
            </div>
        )
